...
| Supervisor | Review an emergency; inspect its submitted schedule; approve, reject or request changes with a reason; create all four account types.                        | Public team directory; all emergency requests and their schedule context; decision outcomes and audit history.                                                                                                       | Workforce assignments, assignment instructions, absence details, worker reports, penalties, planner chat and planner editing/import/export controls. |
| Manager    | Create worker accounts; assign dated work inside scheduled weeks; cancel or replace tasks; review worker reports; assign 0–10 internal points with a reason. | Public team directory; programme allocation required for assignments; all workforce assignments and their instructions; all worker reports, manager decisions and reviewed point totals.                             | Planner editing/import/export controls, planning chat, emergency decisions, and supervisor-only decision actions.                                    |
...
The database role `scheduler` is displayed as **Planner**. The server enforces this visibility contract on every `/api/team/state` response; hiding a screen alone is not relied on for privacy. A planner receives coordination fields only, so assignment instructions are withheld. A supervisor receives emergency data only. A manager receives the workforce records required to assign and review work. A worker receives only records linked to their own account.

Planners and supervisors can create all four account roles from Settings / Team accounts. Managers can create workers only. The public directory contains name, email and role. Passwords, sessions, API keys and setup tokens are never returned by an API. Existing retired roles are not automatically promoted.
