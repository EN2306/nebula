# RailSync — working local application

**New: NebulaX PS1 planner.** Scheduler accounts now open the weekly A/B/C planning workspace. Load the bundled challenge dataset or upload eight CSVs, generate scenarios, inspect the timeline and download the submission files. See [PS1-PLANNER.md](PS1-PLANNER.md) for the implemented rules, public results and limitations. The solver works without an AI key.
