...
import { deploymentConfig, requestOrigin, hostingOptions } from './deployment.mjs';
...
export async function createApp(options = {}) {
  const db = await openDatabase({
    dbPath: options.dbPath || process.env.RAILSYNC_DB || path.join(base, 'data', 'railsync.sqlite'),
    databaseUrl: options.databaseUrl ?? process.env.SUPABASE_DB_URL,
    databaseSsl: options.databaseSsl,
  });
  try {
    return await db.transaction(() => initializeApp(db, options));
  } catch (error) {
    await db.close();
    throw error;
  }
...
async function initializeApp(
 ...
 CREATE TABLE IF NOT EXISTS schedule_previews(id TEXT PRIMARY KEY,body TEXT NOT NULL,expires BIGINT NOT NULL);`);
  // Keep historical track-planning events without importing the retired crew workflow.
  if (!(await db.prepare('SELECT id FROM planning_workspace WHERE id=1').get())) {
    ...
    const legacy = legacyTable
      ? await db.prepare('SELECT body FROM workspace WHERE id=1').get()
      : null;
    ...
    await db
      .prepare('INSERT INTO planning_workspace VALUES(1,?)')
      .run(JSON.stringify({ version: 0, events: retained }));
  }
  const psGet = async () => {
    const row = await db.prepare('SELECT body FROM ps1_workspace WHERE id=1').get();
    return row ? JSON.parse(row.body) : { version: 0, files: null, results: {} };
  };
  const psSave = async (s) =>
    await db
      .prepare(
  ...
  let psBusy = false;
  const get = async () => {
    const row = await db.prepare('SELECT body FROM planning_workspace WHERE id=1').get();
  ...
  const auditAction = async (u, action, detail) => {
    const s = await get();
    audit(s, u, action, null, detail);
    await save(s);
  };
  const team = await operations(db, psGet, psView, auditAction);
  ...
  async function userCreate(b) {
    ...
    await db
      ...
      .run(u.id, name, email, u.password, u.role, u.team_id);
    return u;
  }
  if (process.env.RAILSYNC_DEMO_ACCOUNTS === 'true') {
    const seeded = [];
    for (const account of DEMO_ACCOUNTS)
      if (!(await db.prepare('SELECT id FROM users WHERE email=?').get(account.email)))
        seeded.push(account);
    for (const account of seeded) await userCreate(account);
    if (seeded.length) {
      const s = await get();
  ...
  if (!Array.isArray(initialAccounts) || initialAccounts.length > 20)
    throw Error('Initial accounts must be a JSON array with at most 20 accounts.');
  for (const account of initialAccounts) {
    const email = text(account.email, 'Email', 150).toLowerCase();
    if (!(await db.prepare('SELECT id FROM users WHERE email=?').get(email)))
      await userCreate(account);
  }
  async function login(u, res) {
    const token = randomBytes(32).toString('hex'),
      csrf = randomBytes(24).toString('hex');
    await db.prepare('DELETE FROM sessions WHERE expires<?').run(Date.now());
    await db
      .prepare('INSERT INTO sessions VALUES(?,?,?,?)')
      .run(digest(token), u.id, csrf, Date.now() + 8 * 3600000);
    res.setHeader(
      'Set-Cookie',
    ...
    try {
      const handle = async () => {
        ...
        if (!route.startsWith('/api/')) {
          ...
          responseBody = readFileSync(path.join(base, 'public', files[route]));
          return;
        }
        if (req.method === 'GET' && route === '/api/health') {
          send(200, {
            ok: true,
            setupRequired: !(await db.prepare('SELECT id FROM users LIMIT 1').get()),
            setupTokenRequired: !!setupToken || deployment.secure,
            hosted: deployment.secure,
            ephemeral: db.kind === 'supabase' ? false : ephemeral,
            storage: db.kind,
          });
          return;
        }
        ...
        if (req.method === 'POST' && route === '/api/setup') {
          ...
          const s = await get(),
            u = await userCreate({ ...b, role: 'scheduler' }, s);
          audit(s, u, 'Workspace created', null, 'Planner account created.');
          await save(s);
          send(201, await login(u, res));
          return;
        }
        if (req.method === 'POST' && route === '/api/login') {
          ...
          const email = text(b.email, 'Email', 150).toLowerCase(),
            ...
            u = await db.prepare('SELECT * FROM users WHERE email=?').get(email);
          if (!u || !verify(password, u.password)) fail(401, 'Email or password is incorrect.');
          send(200, await login(u, res));
          return;
        }
        ...
        const u = await db.prepare('SELECT * FROM users WHERE id=?').get(session.user_id);
        ...
        if (route.startsWith('/api/ps1/')) {
          ...
          if (route === '/api/ps1/what-if' && req.method === 'POST') {
            ...
            try {
              ...
              if ((await psGet()).version !== snapshot.version)
        ...
        const teamResult = await team(route, req.method, b, u);
        if (teamResult) {
        ...
        if (route === '/api/state' && req.method === 'GET') {
          send(200, await view(await get(), u));
          return;
        }
          ...
          send(
            ...
            (
              await db
                ...
                .all(u.id, scenario, (await psGet()).version)
            ).reverse(),
          );
          return;
        }
        ...
        if (route === '/api/chat' && req.method === 'POST') {
          ...
          if (!config.key)
            fail(
              503,
              'Connect an API key in Settings to ask questions. Planning works without it.',
            );
          limit('chat:' + u.id, 12, 60000);
          if (aiBusy.has(u.id)) fail(429, 'Wait for your current response.');
          aiBusy.add(u.id);
          try {
            const ps = await psGet(),
            ...
            if ((await psGet()).version !== ps.version)
              fail(
                409,
                'The dataset changed while answering. Ask again using the current dataset.',
              );
            const at = new Date().toISOString(),
              insert = db.prepare(
                'INSERT INTO planning_messages(user_id,scenario,dataset_version,role,content,created) VALUES(?,?,?,?,?,?)',
              );
            await insert.run(u.id, scenario, ps.version, 'user', prompt, at);
            await insert.run(u.id, scenario, ps.version, 'assistant', reply, at);
            send(200, { reply });
          } finally {
            aiBusy.delete(u.id);
          }
        ...
        if (route === '/api/users' && req.method === 'POST') {
          ...
          const created = await userCreate(b);
      ...
      if (req.url.startsWith('/api/'))
        await db.transaction(handle, { readOnly: req.method === 'GET' });
      else await handle();
    } catch (error) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const app = await createApp(hostingOptions()),
